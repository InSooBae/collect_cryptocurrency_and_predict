...
def merge_crypto_gnews_sentiment(crypto_data_filename, google_news_data_filename, output_data_filename):
    ...
    news_df_comb= pd.read_csv(google_news_data_filename[0:-4] + '_with_timestamp.csv', index_col=0)
    news_df_comb.index = pd.to_datetime(news_df_comb.index)
    crypto_df = pd.concat([crypto_df, news_df_comb], axis=1)
    crypto_df.to_csv(output_data_filename)
...
if __name__ == '__main__':
    crypto_data_filename = 'crypto_data_master_cleaned_05_26.csv'
    gnews_data_filename = 'google_news_final_sentiment_05_26.csv'
    crypto_gnews_filename = 'crypto_data_news_final_05_26.csv'
    reddit_data_filename = 'reddit_data_sentiment_bucketized_05_26.csv'
    crypto_gnews_reddit_filename = 'crypto_data_news_reddit_final_05_26.csv'
